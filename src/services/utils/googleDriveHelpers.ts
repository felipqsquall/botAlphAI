...
export async function createSubdirectory(auth: any, parentFolderId: string, subdirectoryName: string): Promise<string> {
...
export async function uploadToGoogleDrive(auth: any, fileName: string, filePath: string, parentFolderId: string): Promise<drive_v3.Schema$File> {
  const driveService = google.drive({ version: 'v3', auth });
